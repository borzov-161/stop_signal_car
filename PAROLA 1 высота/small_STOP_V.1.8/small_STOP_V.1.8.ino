...
uint8_t zadan_dlina = MAX_DEVICES; //количество букв помещающихся на табло//
...
if (val < 800)  {
    ...
    if      (val < 50)  opros=0, textN=1; // сверяем переменную, если val меньше 50 возвращаем 1 (первая кнопка)
    else if (val < 150) opros=0, textN=1; // если val меньше 150 вторая кнопка и т.д.
    else if (val < 350) opros=0, textN=1;
    else if (val < 500) opros=0, textN=1;
    else if (val < 800) opros=0, textN=1;
        ...
        {if (new_taimer - taimer6 >= time_sbros && cikl==1 ) //пауза//если цикл=2, то фраза горит бесконечно
             {textN=3; flag_tel=0; flag_Jstik =0; adrees = 0; opros=0; sbros(); // по истечении паузы сбрасываем  на знак "/! " ///
             }   
        }
...
//------------------обработка фраз с телефона--------------------------------------------------------------
     ...
     if   (str_old) {
           speed=0;  ///быстренько убираем старую строку
           utf8Ascii(msg_tel); // конвектируем кириллицу
          }
     else  speed=skorst; ///возвращаем скорость
     byte lennn=dlina_telef();
     if  (lennn > MAX_DEVICES+2 || iffekt==2) s=2,y=2;//если фраза не помещаеться//или мы хотим ,что бы бежала.
     else if (lennn <= MAX_DEVICES+2 && shtorki==2)  s=0,y=3;//если мы выбрали эфект шторки с телефона
