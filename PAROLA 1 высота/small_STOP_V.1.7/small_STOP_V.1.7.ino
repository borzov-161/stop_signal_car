...
void loop() {
 ...
 //--------------------------------------------------------------------------------
/*uint8_t joystick = analogRead(AnalogPin); uint8_t joystick2 ;
if ( joystick > 200 ) 
   { flag_tel=0; sbros();
   Serial.print("joystick= "); Serial.println(joystick); 
     joystick2 = map(joystick, 0, 1200, 1, 5); //ивертирует диапазон под паролу, после новой команды.
     switch (joystick2) {
     case 1:  opros=0; textN=1; break;
     case 2:  opros=1; textN=0; break;
     case 3:  opros=2; textN=1; break;
     case 4:  opros=3; textN=1; break;
     }
     delay(1000);
  Serial.println(joystick2); 
   }*/
 //==================== прием по блютуз фраз и команд =========================================
if (Serial.available() )  /// если данные пришли по блютуз
