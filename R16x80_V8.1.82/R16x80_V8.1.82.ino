...
uint8_t i=0;        uint8_t y=0;  ///флаг
uint8_t pravo=0;    uint8_t levo=0; ///флаги
uint8_t reklama=0; /// выкл=1, вкл=0 // автоубирание фраз на знак /!\.управляется пультом
uint8_t avariik=0; /// флаг
uint8_t prokrut=0;// переменная хранит количество прокруток фраз до сброса
uint8_t zadan=100; //переменная хранит заданное количество прокруток фраз до сброса
//////////тут ниже меняем под себя//переменная uint8_t не может быть более 255////////////////
uint8_t korotk=100; // тут пишем условно время сброса статичных(коротких)фраз//10примерно =1сек//
uint8_t dlinue=3;  //тут пишем количество прокруток до сброса бегущих фраз
/////////////////////////////////////////////////////////////////////////////////////////////
#define ARRAY_SIZE(a) (sizeof(a)/sizeof(a[0])) ///пересчитываем массив
char *msg[] = /// буквы "Ё" нет.Кириллица только большие буквы.
 {"RUS161",    //0 //
  ...
  "ТАНКИ ГРЯЗИ НЕ БОЯТСЯ",  //11// номера длиннх строк нужно указать ниже где "ХХХХХХХХХХХХХХХ" 
  "S O S",  //12/
  "ПЖЛ АИ92",     //13// 
  "ТОЛКНИТЕ",     //14// 
  "БУКСИР",        //15// 
 ...
 void sbros() {P.setInvert(LOW);tormaz=0;nazad=0;levo=0;pravo=0;skorost=25 ;avariik =0;} // сбрасываем флаги. ждем новых команд
    ////////////////////////////////////////////////////////////////////////////////////////////////
 void priem()  ///////////принимаем команды ифк////////////////////////////////////////////////// 
  {if ( irrecv.decode( &results )) // если данные ифк пришли// запоминаем что
        ...
        case 0xFDB04F:distanc=0;prokrut=0;zumer();break; //0 кнопки пульта
        case 0xFD00FF:distanc=1;prokrut=0;zumer();break; //1//
        case 0xFD807F:distanc=2;prokrut=0;zumer();break; //2//делаем пи пи. куда надо
        case 0xFD40BF:distanc=3;prokrut=0;zumer();break; //3//
        case 0xFD20DF:distanc=4;prokrut=0;zumer();break; //4//
        case 0xFDA05F:distanc=5;prokrut=0;zumer();break; //5//
        case 0xFD609F:distanc=6;prokrut=0;zumer();break; //6//
        case 0xFD10EF:distanc=7;prokrut=0;zumer();break; //7//
        case 0xFD906F:distanc=8;prokrut=0;zumer();break; //8//
        case 0xFD50AF:distanc=9;prokrut=0;zumer();break; //9//
        case 0xFDA857:distanc=20;rejim=0;zumer();break; //  ok  // выклюить текущию фразу. вкл стартовую фразу
        case 0xFD8877:rejim=2;prokrut=0;zumer();break; //  /\  // след фраза
        case 0xFD9867:rejim=3;prokrut=0;zumer();break; //  \/  // предыдущ фраза
        case 0xFD28D7:rejim=4;zumer();break; //<//строка бежит
        case 0xFD6897:rejim=5;zumer();break; //>//эфект шторки
        case 0xFD708F:reklama=1;sbros();zumer();break; //  #  ///нажал " # " фразы не ухадят сами
        case 0xFD30CF:reklama=0;zumer();break; //  *  ///нажал " * " фразы сами уходят
          } 
     ...
     int8_t  speed = skorost;  P.setSpeed(speed);///меняем скорость
      if (P.displayAnimate())  /// выводим результат на табло
        { switch  (opros) { 
         ...
         case 5: mytext=distanc; i=0;y=1; skorost=40;prokrut++; break;///пульт /строка статична
         case 7: mytext=21;      i=0;y=1; tormaz++;break; // стоп 
         case 8: mytext=distanc; i=2;y=2; skorost=30;prokrut++;break; ///  бегущая строка влево.
         case 9: mytext=distanc; i=0;y=4; skorost=40;prokrut++;break; ///  эфект шторки
          } 
  ...
  if (PIND<240)  ///опрашиваем порт. если,что то нажато. проверяем,что 
     ...
     { if(tormaz==0&&digitalRead(4)==LOW)
       ...
       if(levo==1&&digitalRead(7)==HIGH)  // выкл поворот вправо
         {sbros();}  // сбрасываем */
         /////аварийка/////////////////////////////// //////////////////////////////////////      
       if(reklama==0&&avariik==0&&pravo==0&&levo==0&&tormaz==0&&nazad==0&&digitalRead(6)==LOW&&digitalRead(7)==LOW) //2поворота
         {skorost=0; opros=3;avariik=1;}  // аварийка // спасибо
       if(reklama==0&&avariik>=30&&tormaz==0&&nazad==0&&digitalRead(6)==LOW&&digitalRead(7)==LOW) //2поворота
       ...
       if(nazad==1&&digitalRead(5)==HIGH )
        ...
        { if (rejim==0)             { opros=5; zadan=korotk;}    /// выводим фразу от пульта
          if (rejim==2 && mytext<19){ distanc++; rejim=0;}      //  фраза +  ///от пульта
          if (rejim==3 && mytext>0 ){ distanc--; rejim=0;}      //  фраза -  /// от пульта
          if (rejim==4 )            { opros=8;}      //  строка бежит
          if (rejim==5 )            { opros=9;}    /// эфект шторки
    ///ХХХХХХХ прописываем номера длинных строк (ВРУЧНУЮ), чтобы они побежали // ХХХХХХХХХХХХХХХХХХХХХХХХХХХХХХХ     
          if (distanc==10||distanc==11||distanc==17||distanc==18)
              {opros=8;zadan=dlinue;}  ///3 количество прокруток для бегущей строки//статичная прокрутка быстрая
          if (prokrut>zadan && reklama==0 ) //100 задаем для статичной строки//мах 255//т.к. переменная (uint8_t)
             { opros=5;prokrut=0;distanc=20;rejim=0;} ///сбасывае фразу на /!\ стартовую
          }
