...
uint8_t tormaz=0,nazad=0,i=0,y=0,pravo=0,levo=0,avariik=0,desytki=0,reklama=0,flag=0,ctop=0,opros=5,mytext = 21; ////флаги /
...
uint8_t str_n = 18; //количество букв помещающихся на табло//для кириллицы n*2//если помещается 9 букв-пишем str_n = 18//
...
{ /// блок старта (инициализации). самый ПРОБЛЕМАТИЧНЫЙ участок.!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   delay(2017); // копим энергию на "ХОЛДНЫЙ" старт -когда все кондеры разряжены (был долгий перерыв) 
 ...
 //  delay(1000); // сново копим энеергию на пуск табло
...
{ if (distanc!=mytext) //увеличиваем скорость до момента появления новой фразы
  ...
  if ( distanc <1)    /// не менее 0
      { distanc=0;}    
   String MyStr = msg[distanc]; ///создаем  класс String для подсчета длины текущей фразы
   uint8_t str_dlina = MyStr.length(); /// представляем дину строки как целое число
  if   (str_dlina > str_n) //если фраза не помещается//зависит от длины табло//18=9 букв кирилицы.
...
/// если, что то нажато, проверяем что?//по приоритету после первой истины, далее не проверяем (else  if)//
 ...
 else if (tormaz==1&&digitalRead(trm)==HIGH&& ctop>=invers
 
 )  //если прошло хх инверсий, то больше не раздражаем соседа сзади,
