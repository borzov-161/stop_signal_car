...
uint8_t nzd=5;    //назад  // скетч использует 4,5,6,7  пины. если возьмете дугие пины нужно менять  "PIND<16 "
...
unsigned long time_povorot = 700;  // время между сигналами поворотов//тут нет конденсаторов как ранее//
...
{ /// блок старта (инициализации). самый ПРОБЛЕМАТИЧНЫЙ участок.!!!!!!!!!!!
 //delay(2801);
   ...
   P.begin(MAX_ZONES); // инициализация 2 зоны//тут глючит
...
// Serial.println("161");// строка для холодного старта//толи пишем толи нет// индивидуально
  //------------чтение сохранненых переменных их энергонезависимой памяти ----------------------- 
skorst=EEPROM.read(31);jarkost=EEPROM.read(32);invers=EEPROM.read(33); vremya=EEPROM.read(34);iffekt=EEPROM.read(35);shtorki=EEPROM.read(36);
}///
 //=================loop===============================================================
void loop() 
...
if  (flag_tel==1) // Serial.print(Str_telf);
  ...
  if (adrees == 29184 && len>21&&len<26) //если пришла команда разгбераем его на части отделенные запятой
   ...
   if (coxpan==2)  //если пришла команда записать//седьмая позиция в команде
      { EEPROM.update(31, skorst);  /// если значение новое перезаписываем.
        EEPROM.update(32, jarkost); //31,32..36 от фоноря взял //лишбы не первые//
...
//------------------обработка фраз с телефона--------------------------------------------------------------
     ...
     else if (dlina_telef() <= zadan_dlina && shtorki==2)
          {s=0;y=4;}  // эфект шторки
   ...
   if  (P.displayAnimate()) 
    ...
    if (new_taimer - taimer6 >= time_sbros && cikl==1 ) //пауза//если цикл=2, то фраза горит бесонечно
...
/// если, что то НАЖАТО, проверяем что?//по приоритету после первой истины, далее не проверяем (else  if)//
...
else if (tormaz==1&&digitalRead(trm)==HIGH&&(new_taimer - taimer1 >= time_invers))  //если прошло хх сек, а тормоз нажат,то
   ...
   //----------------аварийка-------------------------------------------------------------------------  
         ...
         { opros=0; textN =1;avariik=1; taimer4 = new_taimer; taimer5 = new_taimer;}  // аварийка // спасибо
else if (avariik==1&&digitalRead(prv)==HIGH&&digitalRead(lev)==HIGH) //2поворота
       {  taimer4 = new_taimer;  ///спасибо//перевзводим таймер между морганиями аварийки 
        if (avariik ==1 &&(new_taimer - taimer5 >= time_sbros)) //выкл СПАСИБО ели прошло хх сек
           {opros=3;textN =2;} // поистечении паузы сбрасываем фразу с пульта на знаки   "/!  /!  ///
       }
else if (avariik ==1&&(new_taimer - taimer4 >= time_povorot)) //больше не моргает аварийка
    ...
    //------------нажат поворот влево--------------------------------------------
         ...
         { opros=2;textN =6;levo=1;taimer2 = new_taimer;}  // СТРЕЛКИ бегут влево
...
else if (levo==1&&digitalRead(lev)==LOW&&(new_taimer - taimer2 >= time_povorot))  // выкл поворот вправо
         {sbros(); } // сбрасываем 
   //-------------нажат поворот вправо------------------------------------------
else  if (pravo==0&&avariik ==0&&digitalRead(prv)==HIGH&&digitalRead(lev)==LOW)// 
          { opros=1;textN =5;pravo=1;taimer7 = new_taimer;}  // СТРЕЛКИ бегут вправо
else if (pravo==1&&digitalRead(prv)==HIGH&&digitalRead(lev)==LOW)// 
          {taimer7 = new_taimer;}  // СТРЕЛКИ бегут вправо//певзводим таймер3 
else if (pravo==1&&digitalRead(prv)==LOW&&(new_taimer - taimer7 >= time_povorot))  // выкл поворот вправо
          {sbros(); } // сбрасываем 
 ...
 if (flag_tel==0)  
    {    switch  (opros) /// 
      { case 0: s=0;y=1; speed=skorst;  break;  //строка стaтична
        case 1: s=0;y=4; speed=skorst+5;  break;  //стрелки вправо//используем эфект шторки (половину его)
        case 2: s=0;y=4; speed=skorst+5;  break;  //стрелки влево// используем эфект шторки (половину его)
        case 3: s=0;y=4; speed=skorst+15; break;//назад и аварийка//эфк шторки//чуть помедленее кони.
       default: s=0;y=1; textN=0;  //строка стaтична
      }
