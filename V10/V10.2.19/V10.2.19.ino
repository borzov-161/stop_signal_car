...
uint8_t s=0,y=0,eeprom=0,text_old=3,opros=0,textN =3,len=25; ////флаги //
boolean tormaz=0,nazad=0,pravo=0,flag_tel=0, //флаги //
        levo=0,avariik=0,ctop=0; ////флаги //
...
/// если, что то НАЖАТО, проверяем что?//по приоритету после первой истины, далее не проверяем (else  if)//
   //---------------------стоп --------------------------------------------------------------------
       ...
       { if (digitalRead(trm)&& ctop>=invers)  //если прошло хх инверсий, то больше не раздражаем соседа сзади,
            {P.setInvert(LOW);} // Стоп не инвертируется далее
         if (digitalRead(trm)&&(new_taimer - taimer1 >= time_invers))  //если прошло хх сек, а тормоз нажат,то
            {taimer1 = new_taimer; P.setInvert(!P.getInvert()); ctop++;} // инвертируем СТОП
         if (!digitalRead(trm))  /// отпущен тормоз, сброс
            {sbros();} /// 
       }
 //-----------------назад-------------------------------------------------------------------------  
